...
test('the last daily refresh of the month is not a rewrite', () => {
  assert.equal(rewrittenAfterClose('202608', '20260831T130427Z'), false);
});

test('a copy written after the month closed is a rewrite', () => {
  // 2026-08-10 rewrote June and July, which is the real finding shape.
  assert.equal(rewrittenAfterClose('202607', '20260810T213129Z'), true);
  assert.equal(rewrittenAfterClose('202606', '20260810T213129Z'), true);
});
...
test('the grace covers a refresh that lands just into the next month', () => {
  ...
  assert.equal(rewrittenAfterClose('202608', '20260901T130000Z'), false);
  assert.equal(rewrittenAfterClose('202608', '20260902T130000Z'), false);
  assert.equal(rewrittenAfterClose('202608', '20260905T130000Z'), true);
});
...
test('an unparseable stamp is not reported as a finding', () => {
  ...
  assert.equal(rewrittenAfterClose('202608', 'unknown'), false);
});
